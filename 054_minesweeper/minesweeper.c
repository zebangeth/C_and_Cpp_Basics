...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  game_board->height = h; 
  game_board->totalMines = numMines; 
  game_board->width = w; 
  game_board->board = malloc(h * sizeof(*game_board->board)); 

  for (int j = 0; j < h; j++) {
    game_board->board[j] = malloc(w * sizeof(**game_board->board));
      for (int i = 0; i < w; i++) {
        game_board->board[j][i] = UNKNOWN;
      }
    }

  for (int i = 0; i < numMines; i++) {
    addRandomMine(game_board);
  }
